...
module Broadcasts
  class UsersList < Base
    ...
    def user_joined(round_user:, user_voted:)
      broadcast_prepend_later_to round_channel(round_user.round_id),
                                 target: 'users_list',
                                 html: render(VIEW_COMPONENT_KLASS.new(round_user:, user_voted:))
    end

    def user_left(round_user:)
      broadcast_remove_to round_channel(round_user.round_id),
                          target: round_user_frame_tag(round_user.id)
    end

    def estimation_added(round_user:)
      broadcast_update_later_to round_channel(round_user.round_id),
                                target: round_user_frame_tag(round_user.id),
